...
**전제 조건**: 체크포인트/재시작은 source 테이블에 정렬 가능한 유니크 키 컬럼(예: PK)이 있다고 가정한다 (키셋 방식 — `WHERE key > ? ORDER BY key`로 재개 지점 이후를 이어 읽음). 관계형 DB에서는 일반적인 전제라 MVP 범위에서는 문제없음.
...
- 다중 DB 조합을 유연하게 지원 (드라이버 추상화) — CheckpointStore/재시작이 "정렬 가능한 유니크 키 컬럼으로 이어읽기(키셋 방식)"를 전제로 하는데, 이건 관계형 DB 한정 가정. NoSQL(MongoDB의 _id, Cassandra/DynamoDB의 파티션 키+토큰 페이지네이션 등)로 확장하려면 재개 지점 표현 방식을 소스 타입별로 추상화하는 공통 Reader 인터페이스가 필요
- CSV → DB 이관 확장
